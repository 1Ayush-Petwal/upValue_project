...
def main():
    ...
    if st.session_state.results:
        ...
        with tab2:
            ...
            # Show all signals including HOLD
            signal_stocks = df_display.copy()  # Remove the filter for non-HOLD signals

            if not signal_stocks.empty:
                cols = st.columns(3)  # Changed to 3 columns to include Hold signals
                with cols[0]:
                    st.metric("Buy Signals", len(signal_stocks[signal_stocks['Action'] == 'BUY']))
                with cols[1]:
                    st.metric("Sell Signals", len(signal_stocks[signal_stocks['Action'] == 'SELL']))
                with cols[2]:
                    st.metric("Hold Signals", len(signal_stocks[signal_stocks['Action'] == 'HOLD']))

                # Calculate average potential return only for BUY/SELL signals
                active_signals = signal_stocks[signal_stocks['Action'] != 'HOLD']
                if not active_signals.empty:
                    avg_potential = active_signals['Potential Return %'].mean()
                    st.metric("Avg. Potential Return (Active Signals)", f"{avg_potential:.2f}%")
                    ...
                    with st.expander(f"{symbol} - {row['Action']}", expanded=True):
                        ...
                        with cols[0]:
                            ...
                            st.metric("Expected Price", f"{currency}{row['Expected Price']}", 
                                     delta=f"{row['Potential Return %']}%" if row['Action'] != 'HOLD' else None)
                ...
                st.info("No trading signals found for the current stocks and parameters.")
